...
class StrokeService:
    def __init__(self):
        self.stroke = pd.read_csv('C:/Users/AiA/PycharmProjects/flaskproject - 복사본/static/data/dam/ml/data/healthcare-dataset-stroke-data.csv')
        self.my_stroke = None
    ...
    def norminal(self):
        ...
        self.spec()
        print(" ### 프리프로세스 종료 ### ")
        self.stroke.to_csv("C:/Users/AiA/PycharmProjects/flaskproject/static/data/dam/ml/data/stroke.csv", index=False)
    ...
    '''
    def target(self):
        df = pd.read_csv('C:/Users/AiA/PycharmProjects/flaskproject/static/data/dam/ml/data/stroke.csv')
        self.data = df.drop(['뇌졸중'], axis=1)
        self.data = self.data.drop(['아이디'], axis=1)
        self.targeting = df['뇌졸중']
        print(f'--- data shape --- \n {self.data}')
    ...
    def learning(self, flag):
        ...
        if flag == 'ccee':
            ...
            tree = DecisionTreeClassifier(criterion="gini", random_state=0, max_depth=4)
            parmas = {'criterion': ['gini', 'entropy'], 'max_depth': range(1, 21)}
            # 5회의 교차검증을 2개의 기준마다 20개의 max_depth 값으로 대입
            grid_tree = GridSearchCV(tree, param_grid=parmas, scoring='accuracy', cv=5, n_jobs=-1, verbose=1) #cv => 교차 검증 파라미터 ?회 실시, n_jobs => 멀티코어 cpu 모두 사용, verbose => 연산중간 메세지 출력
            grid_tree.fit(X_train, y_train)
            tree.fit(X_train, y_train)
            print("Accuracy on training set : {:.5f}".format(grid_tree.best_score_))
            print("Accuracy on test set : ", (grid_tree.best_params_))
            best_clf = grid_tree.best_estimator_
            pred = best_clf.predict(X_test)
            print("Accuracy on training set : {:.5f}".format(accuracy_score(y_test, pred)))
